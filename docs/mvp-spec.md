...
BridgePoint の一次価値は「運用担当が迷わず・壊さず・すぐ回せること」とする。
本アプリの差別化は以下の 2 点を中核に据える。

- 失敗しない設計（誤操作・二重付与・設定不備を予防する）
- 移行しやすさ（将来の上位施策や他システム移行を阻害しない）

また、BridgePoint は「固定ロジックを押し付けるアプリ」ではなく、Shopify Flow を柔軟性のエンジンとして運用に合わせて育てられる設計思想を採る。
...
- 月次の付与額 / 利用額 / 失効額の表示
- 最小限の設定画面
- 管理画面内 About ページ（「ポイントプログラムとは？」）の提供
- New customer accounts 非対応ストアでも導入・運用開始できる構成
...
### 10.5 About（ポイントプログラムとは？）

運用担当者とマーチャントが、制度の前提を誤解なく理解できるようにする説明ページ。

- 本アプリのポイントは Store Credit ベースであること
- 付与ルール（いつ・何を基準に・どの単位で付与するか）
- 失効ルール（有効期限と失効の扱い）
- できること / できないこと（v1 の対象範囲）
- checkout 利用条件（利用可能なストア条件）
- 運用上の注意事項（誤操作防止、問い合わせ時の確認観点）
...
- New customer accounts は checkout での Store Credit 利用時に必要となる前提であること
- ただし v1 は New customer accounts 非対応ストアでも「貯める・管理する」を中心価値として導入可能であること
- Flow が利用可能なプランであること
...
- 注文でポイントが貯まる
- （対象ストアでは）checkout で使える
- （非対応ストアでも）運用担当が「貯める・管理する」を開始できる
- 運用担当が制度を誤解しないための About ページが提供されている
- 失敗しない設計と移行しやすさが、画面と運用導線で担保されている
- 付与ごとに有効期限が持てる
- 顧客ごとの残高と履歴が見える
- 管理画面から手動付与できる
- ストア全体の付与・利用・失効が見える
